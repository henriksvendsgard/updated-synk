...
StyleDictionary.registerFormat({
	...
	format: async ({ dictionary, file, platform }) => {
		...
		output += dictionary.allTokens
			...
			.map((token) => {
				const value = usesReferences(token.original.value) ? formatValueAsScssVar(token.original.value) : token.value;
...
const variableFormatter =
	...
	async ({ dictionary, file }) => {
		...
		const otherVariables = dictionary.allTokens.filter((token) => !token.path.some((word) => ["light", "dark"].includes(word)) && !token.filePath.includes("legacy"));
...
StyleDictionary.registerFilter({
	name: "isBaseVariable",
	filter: (token) => {
		const baseCategories = ["brand", "functional", "spacing", "typography", "unit"];
...
const myStyleDictionary = new StyleDictionary({
	...
	ignore: ["src/core/tokens/color/reference.*.json"],
	platforms: {
		ts: {
...
StyleDictionary.registerFormat({
	...
	format: async ({ dictionary, file, platform }) => {
		...
		output += dictionary.allTokens
			.filter((token) => token.path.includes("light"))
			.map((token) => {
				const key = [...token.path.slice(1).filter((step) => !["dark", "light"].includes(step))].join("-");

				const value = [prefix, ...token.path.filter((step) => !["dark", "light"].includes(step))].join("-");
